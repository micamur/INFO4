...
public class Producer extends Thread {
	...
	@Override
	public void run() {
		// On produit des Message tant qu'il en reste à produire
		while (nbMsg > 0) {
			// S'il y a de la place dans le buffer on produit un Message
			// Sinon on attend 1 milliseconde avant de réessayer
			if (!buffPC.isFull()) {
				buffPC.put(prod());
			} else {
				try {
					Thread.sleep(1);
					if (Options.ECHO_PROD_SLEEP)
						System.out.println("prod[" + idProd + "] attend car buffer plein.");
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			}
