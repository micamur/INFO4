...
int main() {
        ...
        glfwSetWindowTitle( "Polytech RICM 4 - TP3b" );
        ...
        m.normalize(); // permet de voir l'objet en entier
        m.colorize(); // permet d'avoir des couleurs pour les objets noirs
        m.vertices.data(); // accès au tableau de positions
        m.normals.data(); // accès au tableau de normals
        m.faces.data(); // accès au tableau d'indices
        m.colors.data(); //accès au tableau de couleurs
        ...
        GLuint uniformID = glGetUniformLocation(programID, "uni");
        cout << "uni = " << uniformID << endl;

        GLuint curTimeID = glGetUniformLocation(programID, "cur_time");
        cout << "cur_time = " << curTimeID << endl;
        ...
        do {
                ...
                sprintf(title,"Polytech RICM 4 - TP3b - %2.0f FPS",1.0/ delta_time);
                ...
                // TOP LEFT
                // Dessin dans une zone de la fenetre
                ...
                glUniform1i(curTimeID, cur_time);

                // TOP RIGHT
                // Dessin dans une zone de la fenetre
                glViewport((w/2)-1, (h/2)-1, (w/2)-1, (h/2)-1);
                // Fonction de dessin à partir des maillages
                glDrawElements(GL_TRIANGLES, m.faces.size(), GL_UNSIGNED_INT, 0);
                glUniform1i(uniformID, 2);
                glUniform1i(curTimeID, cur_time*2);

                // BOTTOM RIGHT
                // Dessin dans une zone de la fenetre
                glViewport((w/2)-1, 0, (w/2)-1, (h/2)-1);
                // Fonction de dessin à partir des maillages
                glDrawElements(GL_TRIANGLES, m.faces.size(), GL_UNSIGNED_INT, 0);
                glUniform1i(uniformID, 3);
                glUniform1i(curTimeID, cur_time*3);
                ...
                // Fonction de dessin à partir des maillages
                glDrawElements(GL_TRIANGLES, m.faces.size(), GL_UNSIGNED_INT, 0);
                glUniform1i(uniformID, 4);
                glUniform1i(curTimeID, cur_time*4);
